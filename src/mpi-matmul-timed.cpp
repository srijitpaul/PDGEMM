...
int main(int argc, char **argv) {
    ...
    DGEMM_simd(params.tileWidth, params.tileWidth, params.tileWidth, alpha,
...
//    }	
    ...
    // ----Prints the maximum time recorded by each thread.----
    
    double elapsed_time = t2 - t1;
    double* time_data = new double[mpiInfo.world_size];
    MPI_Gather(&elapsed_time, 1, MPI_DOUBLE, time_data, 1, MPI_DOUBLE, 0,
            MPI_COMM_WORLD);
    if(mpiInfo.world_rank == 0) {
        double max_time = elapsed_time;
        for (int i = 1; i < mpiInfo.world_size; i++) {
            if(max_time > time_data[i]) {
